...
import type { InvoicePdfRenderer } from "../types";

/**
 * Draws the "Classic" design into the jsPDF document: logo + business name
 * top-left, "INVOICE" + meta top-right, Bill from / Bill to columns, a
 * dark-headed line-items table (auto-paginating) and a right-aligned totals
 * block with a rule above the balance due. This is a 1:1 port of the
 * original single-function PDF exporter — same coordinates, same fonts.
 */
export const renderClassicPdf: InvoicePdfRenderer = async ({
  doc,
  inv,
  totals,
  pageWidth,
